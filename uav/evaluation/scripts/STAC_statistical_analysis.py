# -*- coding: utf-8 -*-
# source: https://github.com/citiususc/stac/blob/master/stac/nonparametric_tests.py
# added the fixed_friedman_test function
...
def fixed_friedman_test(*args):
    """Fixed friedman_test without using deprecated functions such as scipy.mean, uses numpy instead."""
    k = len(args)
    if k < 2: raise ValueError('Less than 2 levels')
    n = len(args[0])
    if len(set([len(v) for v in args])) != 1: raise ValueError('Unequal number of samples')
